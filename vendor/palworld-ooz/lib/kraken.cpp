...
Modified by Palworld Save Reader contributors on 2026-07-13 to remove
the LZNA and Bitknit decoder paths. This fork supports Oodle decoder types
6 (Kraken), 10 (Mermaid), and 12 (Leviathan).
...
const byte *Kraken_ParseHeader(KrakenHeader *hdr, const byte *p) {
  ...
  if ((b & 0xF) == 0xC) {
    ...
    if (hdr->decoder_type != 6 && hdr->decoder_type != 10 && hdr->decoder_type != 12)
      return NULL;
    return p + 2;
  }
